...
#    KLD = 0.5*tf.reduce_sum(tf.square(mu_encoder)+tf.square(logvar_encoder)-tf.log(tf.square(logvar_encoder))-1,1)
    ...
    x_hat = tf.nn.sigmoid(tf.matmul(hidden_decoder, W_decoder_hidden_reconstruction) + b_decoder_hidden_reconstruction)
...
#    loss_summ = tf.summary.scalar("lowerbound", loss)
    ...
    x_hat_1 = tf.nn.sigmoid(tf.matmul(hidden_decoder_1, W_decoder_hidden_reconstruction) + b_decoder_hidden_reconstruction)
