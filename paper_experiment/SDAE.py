...
@author: zhouying
"""
def mysdae(data,epoch,x_test,stack_size = 2,hidden_size = [25,10],keep_rate= 0.5, scale = 0.1):
...
#            avg_cost = 0.
        ...
        # Loop over all batches
            for k in range(total_batch):
                batch_xs = mydata.next_batch(batch_size)[0]

            # Fit training using batch data
                cost = sdae[j].partial_fit(batch_xs)
                if k % 20 == 0:
                    print("Step {0} | Loss: {1}".format((epoch*total_batch+k), cost))
